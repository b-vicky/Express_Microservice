...
export class Users {
  ...
  static async GetUserProjects(req: Request, res: Response) {
    const createUser = new Users();
    return createUser.getUserProjects(req, res);
  }
  ...
  async userSignin(request: Request, response: Response) {
    ...
    if (!user) {
      ...
      const passMatch = bcrypt.compareSync(request.body.password, user.password);
  ...
  async getUserProjects(request: Request, response: Response) {
    const userId = request?.query?.id;;
    const [failureToFetchUser, userData] = await to(usersModel.findById(userId));
    if (!failureToFetchUser) {
      const [failureToFetchProjects, projectData] = await to(
        projectsModel.find({
          _id: { $in: userData?.projects },
        })
      );
      if (!failureToFetchProjects) {
        return response.send(projectData);
      }
    }
    return response.status(500).send(failureToFetchUser?.message);
  }
}
